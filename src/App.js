  ...
  const [index, setIndex] = useState(0)
   ...
   const getQuestions = async (category, dificulty, type) => {
      ...
      if(category && dificulty && type){
        const APi_call = await axios.get(`https://opentdb.com/api.php?amount=10&category=${category}&difficulty=${dificulty}&type=${type}`);
        ...
        const APi_call = await axios.get(`https://opentdb.com/api.php?amount=10&difficulty=${dificulty}&type=${type}`);
        const response = APi_call.data.results;
        setData(response);

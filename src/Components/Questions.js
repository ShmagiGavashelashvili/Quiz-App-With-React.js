...
const nextHandler = () => {
    ...
    setAnswered(false)
     ...
     return (
         <>
        {totalAnswer < data.length  ?  (<li className='questions'>
            <p className='question-title' dangerouslySetInnerHTML={{ __html: currentQuestion.question}}></p>
              ...
              <div className='btn-container'>
                 {shuffledAnswers.map((answ, indx) => (
                     <button
                      ...
                      className={`
                      ${!answered && selectedIndx === indx ? 'selected' : 
                        answered && correctIndx === indx ? 'winner' : 
                        answered && selectedIndx === indx && correctIndx !== indx ? 'lose' : ''
                    }`}
                    disabled={answered}>
                     </button>
                 ))}
     ...
     </li>) : totalAnswer === data.length  ? (<div className='notification'>
     <p className='notification-msg'>{`you have ${correctAnswer} correct answers out of ${totalAnswer}`}</p>
         <button className='notification-btn' onClick={quizAgain}>Quiz Again?</button>
     </div>) : ''}
     </>
   )
 }
